...
<div class="row page-titles">
    <div class="col-md-5 align-self-center">
        <h4 class="text-themecolor">Contributors</h4>
    </div>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <table id="myTable" class="table table-bordered">
                    ...
                    <tbody>
                    	<?php $no = 1; ?>
                        <?php foreach( $qw_contributor->result() as $row ): ?>
	                        <tr>
	                            <td><?php echo $no++; ?></td>
	                            <td><?php echo $row->contributor_name; ?></td>
	                            <td><?php echo $row->contributor_email; ?></td>
                                <td><?php echo _jk($row->contributor_jk); ?></td>
                                <td><?php echo $row->contributor_phone; ?></td>
                                <td>
                                    <a data-toggle="tooltip" data-placement="right" data-original-title="Reset password. Default password to email" href="<?php echo base_url(); ?>admin/contributor/reset/<?php echo $row->contributor_id; ?>" class="btn btn-warning"><i class="ti-reload"></i></a>
                                </td>
                                <td>
                                    <?php if( $this->db->get_where('library', [ 'library_actor' => 'contributor' ,'actor_id' => $row->contributor_id ])->num_rows() ): ?>
                                    <?php else: ?>
                                        <a href="<?php echo base_url(); ?>admin/contributor/del/<?php echo $row->contributor_id; ?>" class="btn btn-danger"><i class="ti-trash"></i></a>
                                    <?php endif; ?>
                                </td>
	                        </tr>
	                    <?php endforeach; ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Create new contributor account</h4>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
            </div>
            <form action="<?php echo base_url() ?>admin/contributor/add" method="post" novalidate> 
